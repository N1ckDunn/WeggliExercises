...
int main()
{
   ...
   void SomeClass::SomeMethod(uint32_t data_in) {

      // All fields to be default constructed
      RtpParameters curr_params = media_channel_->GetRtpSendParameters(ssrc_);
      D_CHECK_LE(init_params.encodings.size(), curr_params.encodings.size());
      for (size_t i = 0; i < curr_params.encodings.size(); ++i) {
